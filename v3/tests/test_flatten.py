...
class TestBookTruth(unittest.TestCase):
    """Owner, 2026-08-25: the exchange UI showed a 14-level 1c-spread
    book while every book we store caps at 4-5 levels. The client now
    probes both book endpoints with the same markets at boot, keeps
    whichever shows more of the real book, and the choice is a logged
    measurement."""

    def _client(self, legacy_levels, orderbook_levels):
        from v3.api import Client
        c = Client.__new__(Client)
        c.book_source = "legacy"
        def fake_get(url, **kw):
            n = (orderbook_levels if "/v1/orderbook/" in url
                 else legacy_levels)
            lv = [{"px": 0.5 - i * 0.01, "qty": 10} for i in range(n)]
            la = [{"px": 0.52 + i * 0.01, "qty": 10} for i in range(n)]
            return {"book": {"bids": lv, "asks": la}}
        c.get = fake_get
        return c

    def test_a_deeper_orderbook_wins_and_is_reported(self):
        c = self._client(legacy_levels=2, orderbook_levels=8)
        rep = c.probe_book_sources(["m1", "m2"])
        self.assertEqual(rep["chosen"], "orderbook")
        self.assertEqual(c.book_source, "orderbook")
        self.assertEqual(rep["legacy_levels"], 8)      # 2+2 per market
        self.assertEqual(rep["orderbook_levels"], 32)
        self.assertEqual(len(rep["per_slug"]), 2)

    def test_equal_depth_keeps_the_legacy_path(self):
        c = self._client(legacy_levels=4, orderbook_levels=4)
        rep = c.probe_book_sources(["m1"])
        self.assertEqual(rep["chosen"], "legacy")

    def test_a_broken_orderbook_endpoint_changes_nothing(self):
        from v3.api import Client
        c = Client.__new__(Client)
        c.book_source = "legacy"
        def fake_get(url, **kw):
            ...
            return {"book": {"bids": [{"px": 0.5, "qty": 1}],
                             "asks": [{"px": 0.52, "qty": 1}]}}
        c.get = fake_get
        rep = c.probe_book_sources(["m1"])
        self.assertEqual(rep["chosen"], "legacy")
        b = c.book("m1", fetched_at=1.0)     # and book() still serves
        self.assertTrue(b.bids and b.asks)

    def test_orderbook_preference_falls_back_when_it_errors_live(self):
        from v3.api import Client
        c = Client.__new__(Client)
        c.book_source = "orderbook"
        def fake_get(url, **kw):
            if "/v1/orderbook/" in url:
                raise RuntimeError("down")
            return {"book": {"bids": [{"px": 0.4, "qty": 5}],
                             "asks": [{"px": 0.6, "qty": 5}]}}
        c.get = fake_get
        b = c.book("m1", fetched_at=1.0)
        self.assertEqual(b.bids[0][0], 0.4)   # legacy answered
